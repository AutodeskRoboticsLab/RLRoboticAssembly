...
# print texts in green in terminal 
def prGreen(skk): print("\033[92m {}\033[00m" .format(skk))

# filter out invisible files and sub-folders
def file_filter(names):
    filtered_files = []
    for name in names:
...
class ReplayActor(object):
    ...
    def __init__(self, num_shards, learning_starts, buffer_size,
                 ...
                 # added for dynamic experience replay
                 human_demonstration, multiple_human_data, human_data_dir, 
                 dynamic_experience_replay, demonstration_zone_percentage, robot_demo_path):
    ...
    # added for dynamic experience replay
    def load_human_demo(self):
        if self.human_demonstration:
            assert self.human_data_dir is not None, "human demonstration path is None"
            assert (self.multiple_human_data == -1 or 
                self.multiple_human_data == 0 or 
                self.multiple_human_data == 1), "multiple_human_data has to be either -1, 0, or 1"

            # one shot in all buffers 
            if self.multiple_human_data == -1:
                in_file = open(self.human_data_dir, 'rb')
                human_data = pickle.load(in_file)
                in_file.close()
                prGreen("Human demo data loaded from: {}".format(self.human_data_dir))

            # all the shots in one buffer 
            elif self.multiple_human_data == 0:
                names = os.listdir(self.human_data_dir)
                filtered_files = file_filter(names)
                prGreen("Insert {} demonstrations to the buffer".format(len(filtered_files)))
                
                human_data = deque()
                human_demo_dir = None
                for i in range(len(filtered_files)):
                    human_demo_dir = self.human_data_dir + str(i)
                    in_file = open(human_demo_dir, 'rb')
                    human_data.extend(pickle.load(in_file))
                    in_file.close()

            # each shot occupies one buffer 
            elif self.multiple_human_data == 1:
                names = os.listdir(self.human_data_dir)
                human_data_dir = self.human_data_dir
                filtered_files = file_filter(names)
                prGreen("Sampling from {} demonstrations".format(len(filtered_files)))
                selected_transition = random.randint(0,len(filtered_files)-1)
                if selected_transition is not None:
                    human_data_dir += str(selected_transition)
                in_file = open(human_data_dir, 'rb')
                human_data = pickle.load(in_file)
                in_file.close()
                prGreen("Human demo data loaded from: {}".format(human_data_dir))

            def new_buffer():
                return PrioritizedReplayBuffer(
                    self.buffer_size, alpha=self.prioritized_replay_alpha, 
                    permanent_data_length=len(human_data), human_data=human_data, 
            ...
            prGreen("{} human transitions are loaded to the replay buffer.".format(len(human_data)))
    ...
    def update_demos(self):
        # randomly selecting a demo
        robot_data_dir = self.robot_demo_path
        names = os.listdir(robot_data_dir)
        filtered_files = file_filter(names)

        if len(filtered_files) > 0:
            selected_transition = filtered_files[random.randint(0,len(filtered_files)-1)]
            robot_data_dir += str(selected_transition)
            in_file = open(robot_data_dir, 'rb')
            demo_data = pickle.load(in_file)
            in_file.close()

            for policy_id, replay_buffer in self.replay_buffers.items():
                replay_buffer.update_demos(demo_data)
                prGreen("Newly sampled demo length: {}".format(len(demo_data)))
                prGreen("The current demonstration size: {}".format(replay_buffer._permanent_data_length))
