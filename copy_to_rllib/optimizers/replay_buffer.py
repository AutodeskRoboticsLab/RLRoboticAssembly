...
class ReplayBuffer(object):
    @DeveloperAPI
    def __init__(self, size, permanent_data_length=0):
    ...
    def add(self, obs_t, action, reward, obs_tp1, done, weight):
        ...
        if self._next_idx + 1 >= self._maxsize:
            ...
            # added for dynamic experience replay
            self._next_idx = self._permanent_data_length
        else:    
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def __init__(self, size, alpha, 
        # added for dynamic experience replay
        permanent_data_length=0, human_data=None, 
        ...
        # added for dynamic experience replay
        super(PrioritizedReplayBuffer, self).__init__(size, permanent_data_length)
        ...
        if dynamic_experience_replay and demonstration_zone_percentage > 0:
            self._max_demo_size = math.ceil(size * demonstration_zone_percentage) # assign the size for demonstration zone 
            assert self._max_demo_size > 10000, "demonstration zone is too small, please increase buffer size" 
            prGreen("the maximum demonstration zone size: {}".format(self._max_demo_size))
        if self._permanent_data_length > 0:
            # insert human demos to the buffer with full prioritization
            for transition in human_data:
                self.add(transition[0], transition[1], transition[2], transition[3], transition[4], 1)        
